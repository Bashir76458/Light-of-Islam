...
function updateRamadanCounter() {
  const ramadanStart = new Date('2025-03-03T00:00:00');
  const now = new Date();
  const diffDays = Math.floor((now - ramadanStart) / (1000 * 60 * 60 * 24)) + 1;
  document.getElementById('ramadan-day').innerText = diffDays > 0 ? diffDays : "Not started";
}
...
document.getElementById('newHadith').addEventListener('click', function() {
